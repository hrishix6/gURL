...
type HttpTransformer struct {
	...
	MAX_RESPONSE_LIMIT_BYTES int64
}

func NewHttpTransformer(mimeRepo *dbPkg.MimeRepository, maxResponseBytesLimit int64) *HttpTransformer {
	return &HttpTransformer{
		mimeRepo:                 mimeRepo,
		MAX_RESPONSE_LIMIT_BYTES: maxResponseBytesLimit,
	}
}
...
func (htf *HttpTransformer) TempStoreResponse(ctx context.Context, id string, res *http.Response, baseDir string) (*models.TempStorageStats, error) {
	...
	limitReader := &io.LimitedReader{
		R: res.Body,
		N: htf.MAX_RESPONSE_LIMIT_BYTES + 1,
	}
	...
	SizeLimitExceeded := writtenBytes > htf.MAX_RESPONSE_LIMIT_BYTES
